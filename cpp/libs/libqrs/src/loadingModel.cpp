...
{    
    ...
    // std::cout<<outputTensorShape[1]<<std::endl;
}

float* qrs_detection::detect(const std::vector<float> &input)
{
    // run inference
    ...
    std::vector<float> results(num_samps*outputTensorShape[1]);
    // constexpr int myres=num_samps*100;
    // std::array<float,  myres> results2;  
    ...
    // std::cout << results[120]<<" "<<results[121] << std::endl;
    // std::cout << results.data()<<" "<<input.size() << std::endl;
